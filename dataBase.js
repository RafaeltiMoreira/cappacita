...
function curarPokemon(id) {
    const pokemonMaisHp = pokemons[id]
    const encherHp = 20
    if (pokemonMaisHp.hp < 100 || pokemonMaisHp.hp >= 0) {
            pokemonMaisHp.hp = pokemonMaisHp + encherHp
    } 
    if (pokemonMaisHp.hp > 100) {
        pokemonMaisHp.hp = 100
    }
    return `${ pokemonMaisHp.nome }: ${ pokemonMaisHp.hp }`
}
